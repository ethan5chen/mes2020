...
$(function () {
  $.ajax({   
    //url: "https://www.easy-mock.com/mock/5d04e3db15cc714d6ec0676d/mes/getRelation",
  ...
  //单一添加下拉改变事件
  worktypeDom.onmousedown = function(){//当按下鼠标按钮的时候
    this.sindex = this.selectedIndex;//把当前选中的值得索引赋给下拉选中的索引
    this.selectedIndex = -1;//把下拉选中的索引改变为-1,也就是没有!
  }
  worktypeDom.onmouseout = function(){//当鼠标移开的时候
    var index = $("#work_type_selection_select").selectedIndex;//获取下拉选中的索引
    if(index == -1){//如果为-1,就是根本没有选
      this.selectedIndex = this.sindex;//就把下拉选中的索引改变成之前选中的值得索引,就默认选择的是之前选中的值
    }
  };
  // worktypeDom.addEventListener('change',function(){
  // });
