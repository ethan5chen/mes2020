var isFromAddRuleButton=getQueryString("isFromAddRuleButton");
//console.log(isFromAddRuleButton);
//功能型的模态框
...
$(document).on('click', "#select-save-button", function () {
  ...
  $.each(element_select, function (index, item) {
    ...
    for (i = 0; i < attr_item.length; i++) {
      if (attr_item.options[i].selected) {
        // alert((attr_item[i].value).split("-")[1]);
        str.push(item.value + "." + (attr_item[i].value).split("-")[1]);
      }
    }
    //遍历获取到的属性值数组
    $.each(str, function (index, item) {
      let tableName = item.split("-",3)[1];
      let attr_value = item.split(".", 2)[1];
      attr_value = tableName + "." + attr_value;
  ...
  let orderString = "";
  //当排序操作面板中没有任何元素时不添加order by语句排序
