...
function add(){
  if($("#addForm").valid())
 $.ajax({
   url:url+"/dataDict/addEquip.action",
   dataType:"json",
   type:'post',
   data:{
        //method:"addRoles",
        str1:$("#etId option:selected").val(),
        str2:$("#num").val(),
        str3:$("#name").val(),
        str4:$("#descript").val(),
      },
...
$(document).on('click','.deleteButton',function () {
  ...
  //提示信息
  deletePrompt.append("您是否要删除设备:"+roleName);
});
...
$(document).on('click','#edit-save-button',function () {
	if($("#updateForm").validate().form()){
		$.ajax({
			url:url+"/dataDict/updateEquip.action",
			dataType:"json",
			data:{
        strId:editRoleId,
        str1:$("#newEtId option:selected").val(),
        str2:$("#newNum").val(),
        str3:$("#newName").val(),
        str4:$("#newDescript").val(),
      },
      success:function(result){
       if (result.code === 0) {
        swal(
