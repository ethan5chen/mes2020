        ...
        setTimeout(
          function(){ 
           //$("#table").bootstrapTable('refresh');
            ...
            window.location.href="personal_management.html"
          }, 3000);
      }
    });
        ...
        //if (array.indexOf((item.id.toString())) !== -1) {
          if (item.selected=="true") {
            worker_type_check.append(
              ...
              + " class='workType-check' type='checkbox' checked value='"
              + item.id + "' ><label for=" + item.name + ">" + item.name
              + "</label></div>")
          } else {
            worker_type_check.append(
              "<div class=\"center-block text-center\"><input id=" + item.name
              + " class='workType-check' type='checkbox' value='"
              + item.id + "'><label for=" + item.name + ">" + item.name
              + "</label></div>")
          }
        });
...
$(document).on('click', "#worker-type-confirm-button", function () {
  let roles = "";
  let roleCheckLength=$(".workType-check:checked").length;
  $(".workType-check:checked").each(function (index,item) {
  ...
  $.ajax({
    ...
    success: function (result) {
      if (result.code === 0) {
        $("#table").bootstrapTable('refresh');
        //清空选择框
        $(".workType-check").each(function(index,item){
          item.checked=false;
        });
        swal(
          '分配成功',
  ...
  $.ajax({
    ...
    success: function (result) {
      if (result.code === 0) {
        ...
        //清空选择框
        $(".role-check").each(function(index,item){
          item.checked=false;
        });
        //关闭模态框
